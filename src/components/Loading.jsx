...
const Loading = ({
  ...
  const loadingContent = (
    <div className={`w-full max-w-4xl ${className}`}>
      <div className={`relative overflow-hidden rounded-2xl border border-slate-100 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-bg-subtle)] p-5 ${sizeClasses[size]}`}>
        <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.8s_infinite] bg-gradient-to-r from-transparent via-slate-100/60 dark:via-slate-900/30 to-transparent" />
        <div className="relative z-10 space-y-3">
          <div className="h-4 w-40 rounded bg-slate-150 dark:bg-slate-800" />
  ...
  if (fullScreen) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/95 dark:bg-gray-950/95 p-4">
        {loadingContent}
      </div>
    );
  }
