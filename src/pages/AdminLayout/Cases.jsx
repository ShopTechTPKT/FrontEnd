...
const CaseForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Validation
      if (!formData.name) {
        throw new Error('Tên s?n ph?m là b?t bu?c');
      }
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu v? máy tính',
      }));
    }
  };
...
// Component b?ng danh sách cases
const Cases = memo(
  ({
    activeMenu,
    cases = [],
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving case:', error);
        setError(error.message || 'Không th? luu v? máy tính');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_v_my')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_v_my')}
                value={searchTerm}
        ...
        {!isLoading && localCases.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_v')}</span>
          </div>
        )}
        ...
        {!isLoading && localCases.length > 0 && filteredCases.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.khong_tim_thay_vo_may_tinh_phu_hop')}</span>
          </div>
        )}
            ...
            <table className={`min-w-full border ${currentTheme.table}`}>
              <thead className={currentTheme.tableHeader}>
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">{t('admin.danh_mc')}</th>
                ...
                {filteredCases.map((caseItem, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{caseItem.description || 'Không có mô t?'}</div>
                    </td>
        ...
        {formState.isOpen && (
          <CaseForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm v? máy tính m?i' : 'Ch?nh s?a v? máy tính'}
            theme={theme}
