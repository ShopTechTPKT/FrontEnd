...
// Ánh x? categoryID v?i tên hãng cho danh m?c Tablets
...
const TabletForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: tablet?.unitPrice || '',
    quantity: tablet?.quantity || '',
    categoryId: tablet?.categoryId || '',
    imageUrl: tablet?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu s?n ph?m',
      }));
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={formData.image || t('admin.chn_hnh_nh') || 'Ch?n hình ?nh'}
              ...
              <div className="grid grid-cols-3 gap-2 max-h-96 overflow-y-auto">
                {filteredImages.length > 0 ? (
                  ...
                  <div className="col-span-3 text-center text-gray-400">
                    {t('admin.no_image_found') || 'Không tìm th?y hình ?nh'}
                  </div>
                )}
              </div>
            </div>
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving tablet:', error);
        setError(error.message || 'Không th? luu s?n ph?m');
        setIsLoading(false);
        throw error;
      }
    };
      ...
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">
            {t('admin.tablet_list_title') || 'Danh sách Máy Tính B?ng'}
          </h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tablet_search_placeholder') || 'Tìm ki?m máy tính b?ng...'}
                value={searchTerm}
        ...
        {!loading && !isLoading && localTablets.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('remaining.no_tablet_found')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localTablets.length > 0 && filteredTablets.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_tablet_match')}</span>
          </div>
        )}
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">
                        {tablet.description || t('admin.no_description') || 'Không có mô t?'}
                      </div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(tablet.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {tablet.quantity !== undefined ? tablet.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
          ...
          <TabletForm
            ...
            formTitle={
              formState.formType === 'add'
                ? t('admin.tablet_add_title') || 'Thêm Máy Tính B?ng m?i'
                : t('admin.tablet_edit_title') || 'Ch?nh s?a Máy Tính B?ng'
            }
            theme={theme}
