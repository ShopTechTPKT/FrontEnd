...
// Ánh x? categoryID v?i tên hãng cho danh m?c Headphones
...
const HeadphoneForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: headphone?.unitPrice || '',
    quantity: headphone?.quantity || '',
    categoryId: headphone?.categoryId || '',
    imageUrl: headphone?.imageUrl || '',
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu tai nghe',
      }));
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={formData.image || 'Ch?n ho?c nh?p URL hình ?nh'}
                  onClick={() => setShowImagePicker(true)}
                  onChange={handleChange}
                  name="imageUrl"
                  placeholder={t('admin.nhp_url_hnh_nh')}
                  className={`w-full rounded-lg border px-4 py-2 ${currentTheme.input}`}
                />
                {formData.image && (
                  <div className="mt-2">
      ...
      try {
        ...
        if (formState.formType === 'add') {
          ...
          if (!newProduct.id && !newProduct.productID) {
            throw new Error('API không tr? v? ID s?n ph?m');
          }
        ...
        const errorMessage = error.message.includes('Product not found')
          ? 'S?n ph?m không t?n t?i ho?c dã b? xóa'
          : error.message || 'Không th? luu tai nghe';
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_tai_nghe')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_tai_nghe')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localHeadphones.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_tai')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localHeadphones.length > 0 && filteredHeadphones.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_headphone_match')}</span>
          </div>
        )}
                ...
                {filteredHeadphones.map((headphone, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{headphone.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {formatPrice(headphone.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {headphone.quantity !== undefined ? headphone.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <HeadphoneForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm tai nghe m?i' : 'Ch?nh s?a tai nghe'}
            theme={theme}
...
// Updated: 2025-10-12T16:06:25.519Z

// Updated: 2025-10-12T16:09:02.217Z
