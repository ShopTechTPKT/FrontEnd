...
// Ánh x? categoryID v?i tên hãng cho danh m?c Keyboards
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu Bàn Phím',
      }));
    }
  };
    ...
    const handleSave = async (productData) => {
      try {
        ...
        const errorMessage = error.message.includes('Product not found')
          ...
          : error.message || 'Không th? luu Bàn Phím';
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_bn_phm')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_bn_phm')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localKeyboards.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_bn')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localKeyboards.length > 0 && filteredKeyboards.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_keyboard_match')}</span>
          </div>
        )}
        ...
        {formState.isOpen && (
          <KeyboardForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm Bàn Phím m?i' : 'Ch?nh s?a Bàn Phím'}
            theme={theme}
