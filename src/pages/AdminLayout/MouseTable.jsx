...
// Ánh x? categoryID v?i tên hãng cho danh m?c Mice
...
const MouseForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    quantity: mouse?.quantity || '',
    categoryId: mouse?.categoryId || '',
    imageUrl: mouse?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu chu?t',
      }));
    }
  };
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving mouse:', error);
        setError(error.message || 'Không th? luu chu?t');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_chut')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_chut')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localMouses.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_chut')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localMouses.length > 0 && filteredMouses.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_mouse_match')}</span>
          </div>
        )}
                ...
                {filteredMouses.map((mouse, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{mouse.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(mouse.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {mouse.quantity !== undefined ? mouse.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <MouseForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm Chu?t m?i' : 'Ch?nh s?a Chu?t'}
            theme={theme}
