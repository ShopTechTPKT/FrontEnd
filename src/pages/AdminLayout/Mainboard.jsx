...
// Ánh x? categoryID v?i tên hãng cho danh m?c Mainboards
...
const MainboardForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: mainboard?.unitPrice || '',
    quantity: mainboard?.quantity || '',
    categoryId: mainboard?.categoryId || '',
    imageUrl: mainboard?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu bo m?ch ch?',
      }));
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={formData.image || 'Ch?n hình ?nh'}
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving mainboard:', error);
        setError(error.message || 'Không th? luu bo m?ch ch?');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_bo_mch')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_bo_mch')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localMainboards.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_bo')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localMainboards.length > 0 && filteredMainboards.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_mainboard_match')}</span>
          </div>
        )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                        <div className="max-w-xs truncate">{mainboard.description || 'Không có mô t?'}</div>
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {formatPrice(mainboard.unitPrice)}
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {mainboard.quantity !== undefined ? mainboard.quantity : 'N/A'}
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <MainboardForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm bo m?ch ch? m?i' : 'Ch?nh s?a bo m?ch ch?'}
            theme={theme}
