...
// Ánh x? categoryID v?i tên hãng cho danh m?c PCs
...
const PcForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    imageUrl: pc?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu PC',
      }));
    }
  };
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving PC:', error);
        setError(error.message || 'Không th? luu PC');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_pc')}</h2>
        ...
        {!loading && !isLoading && localPcs.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_pc')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localPcs.length > 0 && filteredPcs.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>Không tìm th?y PC phù h?p.</span>
          </div>
        )}
                ...
                {filteredPcs.map((pc, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{pc.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(pc.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {pc.quantity !== undefined ? pc.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <PcForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm PC m?i' : 'Ch?nh s?a PC'}
            theme={theme}
