...
// Ánh x? categoryID v?i tên hãng cho danh m?c Mousepads
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu bàn di chu?t',
      }));
    }
  };
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving mousepad:', error);
        setError(error.message || 'Không th? luu bàn di chu?t');
        setIsLoading(false);
        throw error;
      }
    };
      ...
      <div className={`p-6 ${currentTheme.container}`}>
                ...
                {filteredMousepads.map((mousepad, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{mousepad.description || 'Không có mô t?'}</div>
                    </td>
        ...
        {formState.isOpen && (
          <MousepadForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm bàn di chu?t m?i' : 'Ch?nh s?a bàn di chu?t'}
            theme={theme}
