...
const PhoneForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    imageUrl: phone?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_in_thoi')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_in_thoi')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localPhones.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_in')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localPhones.length > 0 && filteredPhones.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_phone_match')}</span>
          </div>
        )}
...
// Updated: 2025-10-12T16:06:22.569Z

// Updated: 2025-10-12T16:08:49.208Z
