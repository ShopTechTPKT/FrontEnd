...
// Ánh x? categoryID v?i tên hãng cho danh m?c Gaming Gear
...
const GamingGearForm = ({
  ...
  const [formData, setFormData] = useState({
    name: gamingGear?.name || '',
    description: gamingGear?.description || '',
    unitPrice: gamingGear?.unitPrice || '',
    quantity: gamingGear?.quantity || '',
    categoryId: gamingGear?.categoryId || '',
    imageUrl: gamingGear?.imageUrl || '',
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu gaming gear',
      }));
    }
  };
    ...
    const handleSave = async (productData) => {
      try {
        ...
        const errorMessage = error.message.includes('Product not found')
          ...
          : error.message || 'Không th? luu gaming gear';
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_gaming_gear')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_gaming_gear')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localGamingGear.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_gaming')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localGamingGear.length > 0 && filteredGamingGear.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.khong_tim_thay_gaming_gear_phu_hop')}</span>
          </div>
        )}
                ...
                {filteredGamingGear.map((item, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{item.description || 'Không có mô t?'}</div>
                    </td>
        ...
        {formState.isOpen && (
          <GamingGearForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm Gaming Gear m?i' : 'Ch?nh s?a Gaming Gear'}
            theme={theme}
