...
// Ánh x? categoryID v?i tên hãng cho danh m?c Storage
...
const StorageForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    imageUrl: storage?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu thi?t b? luu tr?',
      }));
    }
  };
...
// Component b?ng danh sách storage
const Storage = memo(
  ({
    activeMenu,
    storage = [],
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving storage:', error);
        setError(error.message || 'Không th? luu thi?t b? luu tr?');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_thit_b')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_thit_b')}
                value={searchTerm}
        ...
        {!isLoading && localStorageItems.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_thit')}</span>
          </div>
        )}
        ...
        {!isLoading && localStorageItems.length > 0 && filteredStorage.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_storage_match')}</span>
          </div>
        )}
                ...
                {filteredStorage.map((storageItem, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{storageItem.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(storageItem.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {storageItem.quantity !== undefined ? storageItem.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <StorageForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm thi?t b? luu tr? m?i' : 'Ch?nh s?a thi?t b? luu tr?'}
            theme={theme}
