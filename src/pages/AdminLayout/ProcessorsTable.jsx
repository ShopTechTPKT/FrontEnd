    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">{t('admin.danh_sch_b_vi')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_b_vi')}
                value={searchTerm}
        ...
        {!isLoading && localProcessors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_b')}</span>
          </div>
        )}
        ...
        {!isLoading && localProcessors.length > 0 && filteredProcessors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_processor_match')}</span>
          </div>
        )}
                ...
                {filteredProcessors.map((processor, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(processor.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {processor.quantity !== undefined ? processor.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
