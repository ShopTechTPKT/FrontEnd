...
// Ánh x? categoryID v?i tên hãng cho danh m?c PSUs
...
const PsuForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: psu?.unitPrice || '',
    quantity: psu?.quantity || '',
    categoryId: psu?.categoryId || '',
    imageUrl: psu?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu ngu?n máy tính',
      }));
    }
  };
  ...
  ({
    ...
    const handleSave = async (productData) => {
      try {
        ...
        console.error('Error saving PSU:', error);
        setError(error.message || 'Không th? luu ngu?n máy tính');
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_ngun_my')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_ngun_my')}
                value={searchTerm}
        ...
        {!loading && !isLoading && localPsus.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_ngun')}</span>
          </div>
        )}
        ...
        {!loading && !isLoading && localPsus.length > 0 && filteredPsus.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_psu_match')}</span>
          </div>
        )}
                ...
                {filteredPsus.map((psuItem, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{psuItem.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(psuItem.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {psuItem.quantity !== undefined ? psuItem.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <PsuForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm ngu?n máy tính m?i' : 'Ch?nh s?a ngu?n máy tính'}
            theme={theme}
