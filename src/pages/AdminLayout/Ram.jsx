...
const RamForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: ram?.unitPrice || '',
    quantity: ram?.quantity || '',
    categoryId: ram?.categoryId || '',
    imageUrl: ram?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Không th? luu RAM',
      }));
    }
  };
    ...
    const handleSave = async (productData) => {
      try {
        ...
        const errorMessage = error.message.includes('Product not found')
          ...
          : error.message || 'Không th? luu RAM';
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">{t('admin.danh_sch_ram')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_ram')}
                value={searchTerm}
        ...
        {!isLoading && localRam.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>{t('admin.khng_tm_thy_ram')}</span>
          </div>
        )}
        ...
        {!isLoading && localRam.length > 0 && filteredRam.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>{t('remaining.no_ram_match')}</span>
          </div>
        )}
                ...
                {filteredRam.map((ramItem, index) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                      <div className="max-w-xs truncate">{ramItem.description || 'Không có mô t?'}</div>
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {formatPrice(ramItem.unitPrice)}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {ramItem.quantity !== undefined ? ramItem.quantity : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <RamForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm RAM m?i' : 'Ch?nh s?a RAM'}
            theme={theme}
...
export default Ram;
// Updated: 2025-10-12T16:06:26.253Z

// Updated: 2025-10-12T16:09:10.823Z
