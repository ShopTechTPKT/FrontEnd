...
// Ánh x? categoryID v?i tên hãng cho danh m?c Monitors
...
const MonitorForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    unitPrice: monitor?.unitPrice || '',
    quantity: monitor?.quantity || '',
    categoryId: monitor?.categoryId || '',
    imageUrl: monitor?.imageUrl || '', // Store image URL
    isLoading: false,
    error: null,
  });
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={formData.imageUrl || 'Ch?n hình ?nh'}
      ...
      <div className={`p-6 ${currentTheme.container}`}>
        {error && (
          <div className="mb-4">
            <StatusNotice
              tone="error"
              title="Không th? x? lý d? li?u monitor"
              message={error}
              actionText="Ðóng"
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">{t('admin.danh_sch_mn_hnh')}</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder={t('admin.tm_kim_mn_hnh')}
                value={searchTerm}
        ...
        {!isLoading && localMonitors.length === 0 && (
          <div className={`h-64 flex items-center justify-center ${currentTheme.emptyState}`}>
            <EmptyState title={t('admin.khng_tm_thy_mn')} className="py-0" />
          </div>
        )}

        {!isLoading && localMonitors.length > 0 && filteredMonitors.length === 0 && (
          <div className={`h-64 flex items-center justify-center ${currentTheme.emptyState}`}>
            <EmptyState title={t('remaining.no_monitor_match')} className="py-0" />
          </div>
        )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`px-6 py-4 text-sm ${currentTheme.secondaryText}`}>
                        <div className="max-w-xs truncate">{monitor.description || 'Không có mô t?'}</div>
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {formatPrice(monitor.unitPrice)}
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                        {monitor.quantity !== undefined ? monitor.quantity : 'N/A'}
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
        ...
        {formState.isOpen && (
          <MonitorForm
            ...
            formTitle={formState.formType === 'add' ? 'Thêm màn hình m?i' : 'Ch?nh s?a màn hình'}
            theme={theme}
