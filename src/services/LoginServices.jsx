...
const parseErrorMessage = (error) => {
  ...
  const backendMessage =
    ...
    errorData.detail || errorData.error_description;
...
const translateErrorMessage = (message) => {
  ...
  if (msg.includes('user_not_existed') || msg.includes('user not existed') ||
    msg.includes('user does not exist') || msg.includes('not found') ||
    msg.includes('does not exist') || msg.includes('no value present')) {
    return 'Email không tồn tại trong hệ thống';
  }
  if (msg.includes('bad credentials') || msg.includes('invalid credentials') ||
    msg.includes('wrong password') || msg.includes('incorrect password') ||
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      try {
        ...
        console.warn('⚠️ Could not fetch full user details:', userError.message);
      }
